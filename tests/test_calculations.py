from perpdex_bot.calculations import estimate_order_spread, estimate_slippage
...
def test_order_spread_uses_average_buy_and_sell_prices() -> None:
    bids = (
        OrderBookLevel(BookSide.BID, price=99, size=1, level_index=0),
        OrderBookLevel(BookSide.BID, price=98, size=1, level_index=1),
    )
    ...
    result = estimate_order_spread(150, 100, bids=bids, asks=asks)

    assert result.complete
    assert result.average_buy_price is not None
    assert result.average_sell_price is not None
    assert result.spread == result.average_buy_price - result.average_sell_price
    assert result.spread_bps is not None
    assert result.spread_bps > 200
